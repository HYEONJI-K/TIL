...
for i in range(0, len(d.data)): # 샘플 순서대로 출력
    print(i+1, d.data[i],d.target[i])   # d.data 특징 벡터 / d.target 레이블
    
"""
샘플을 특징 벡터와 레이블로 표현
특징 벡터 : x로 표기 (d는 특징의 개수로서 특징 벡터의 차원이라 부름)
특징 벡터 : x=(x1,x2,x3,...,xd)

레이블 : 0,1,2,...,c-1의 값 또는 1,2,...,c-1,c의 값 또는 원핫 코드
원핫 코드 : 한 요소만 1인 이진열
...
# s의 fit 함수는 훈련 집합을 가지고 학습을 수행(
# 매개변수로 특징 벡터 iris.data와 레이블 iris.target을 설정)

new_d = [[6.4,3.2,6.0,2.5],[7.1,3.1,4.7,1.35]]

# 테스트 집합
res = s.predict(new_d)              # 101 번째와 51번째 샘플을 변형하여 새로운 데이터 생성
...
테스트 집합 : 학습을 마친 모델의 성능을 측정하는데 쓰는 데이터로서 예측할 때는 특징 벡터 정보만 제공하고, 
예측 결과를 가지고 정확률을 측정할 때 레이블 정도를 사용

* 하이퍼 매개변수 : 모델의 동작을 제어하는 데 쓰는 변수.
최적의 하이퍼 매개변수 값을 자동으로 설정하는 일을 하이퍼 매개변수 최적화라 하는데,
이는 기계 학습의 중요한 주제 중 하나다.
...
**Data Set Characteristics:**
    ...
    :Number of Instances: 150 (50 in each of three classes)               # 150개 데이터 개수
    :Number of Attributes: 4 numeric, predictive attributes and the class # 4개의 특징
